...
import { PELABUHAN_OPTIONS, KONDISI_OPTIONS, TRANSPORTASI_OPTIONS, JENIS_IKAN_OPTIONS, KATEGORI_OPTIONS, TUJUAN_OPTIONS } from '../utils/formStandards'

export default function DistribusiIkanPemasaranKeluar() {
  const [openTip, setOpenTip] = useState(null)
  const [header, setHeader] = useState({ pelabuhan: '', tanggalKeluar: '' })
  const initialRows = useMemo(() => (
    Array.from({ length: 5 }, () => ({ jenisIkan: '', kondisi: '', jumlahKg: '', harga: '', kapalAngkut: '', kategori: '', tujuan: '', perusahaan: '', telepon: '', transportasi: 'Kapal Angkut' }))
  ), [])
  const [rows, setRows] = useState(initialRows)
  const [doShake, setDoShake] = useState(false)

  const TIPS = {
    jenisIkan: 'Pilih jenis ikan sesuai master data.',
    kondisi: 'Pilih kondisi ikan sesuai standar master data.',
    jumlahKg: 'Masukkan berat ikan dalam kilogram. Contoh: 150, 250.5, 1000.',
    harga: 'Masukkan harga per kilogram dalam rupiah. Contoh: 25000, 50000.',
    kapalAngkut: 'Nama/jenis kapal angkut atau armada yang digunakan.',
    kategori: 'Kategori pemasaran: misalnya Lokal, Domestik, Antar Pulau, Ekspor.',
    tujuan: 'Tujuan pemasaran: misalnya Pasar Lokal, Antar Kota/Provinsi, Ekspor.',
    perusahaan: 'Nama perusahaan/entitas yang menerima atau mengelola.',
    telepon: 'Nomor telepon/HP kontak yang valid.',
    transportasi: 'Pilih jenis transportasi sesuai master data. Contoh: Kapal Angkut, Truk Pendingin.',
  }

  const newEmptyRow = () => ({ jenisIkan: '', kondisi: '', jumlahKg: '', harga: '', kapalAngkut: '', kategori: '', tujuan: '', perusahaan: '', telepon: '', transportasi: 'Kapal Angkut' })

  const isRequiredEmpty = (key) => {
    if (key === 'pelabuhan') return !header.pelabuhan
    if (key === 'tanggalKeluar') return !header.tanggalKeluar
    return false
  }
  ...
  const resetForm = () => {
    setHeader({ pelabuhan: '', tanggalKeluar: '' })
    setRows(initialRows)
  }

  const onSave = () => {
    if (isRequiredEmpty('pelabuhan') || isRequiredEmpty('tanggalKeluar')) {
      setDoShake(true)
      setTimeout(() => setDoShake(false), 500)
      return
    }
    const payload = { ...header, items: rows }
    // TODO: integrate with backend API for saving
    console.log('Simpan pemasaran keluar:', payload)
    alert('Data pemasaran keluar siap disimpan (mock).')
  }

  const saveDisabled = isRequiredEmpty('pelabuhan') || isRequiredEmpty('tanggalKeluar')

  return (
    <div className="section-card">
      <AutoBreadcrumb />
      <h2 className="section-title">Distribusi Ikan - Pemasaran Keluar</h2>
      <p className="section-desc">Input pemasaran keluar per pelabuhan dan tanggal, dengan tabel item dinamis.</p>

      {/* Header: Pelabuhan & Tanggal Keluar */}
      <div className="fasilitas-form" style={{ paddingTop: 0 }}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div className={`form-group ${isRequiredEmpty('tanggalKeluar') ? 'error' : ''} ${isRequiredEmpty('tanggalKeluar') && doShake ? 'shake' : ''}`}>
            <label className="form-label">Tanggal Keluar <span className="required-mark">*</span></label>
            <input
              type="date"
              name="tanggalKeluar"
              value={header.tanggalKeluar}
              onChange={handleHeaderChange}
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
                  Kapal Angkut/Armada
                  <span className="tooltip-trigger" role="button" aria-label={TIPS.kapalAngkut} onMouseEnter={() => setOpenTip('kapalAngkut')} style={{ color: '#6b7280', fontWeight: 600, fontSize: 12, marginLeft: 6, cursor: 'help', padding: 0, display: 'inline-block' }}>?</span>
                  {openTip === 'kapalAngkut' && (
                    <div role="dialog" aria-label="Informasi pengisian" style={{ position: 'absolute', top: 6, right: 8, background: '#fff', color: '#111', border: '1px solid #e5e7eb', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', padding: '8px 10px', maxWidth: 280, zIndex: 20 }}>
                      <div style={{ fontSize: 12 }}>{TIPS.kapalAngkut}</div>
                      <button onClick={() => setOpenTip(null)} aria-label="Tutup" style={{ position: 'absolute', top: 4, right: 6, background: 'transparent', border: 'none', color: '#6b7280', cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </th>
                <th style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
                  Kategori
                  <span className="tooltip-trigger" role="button" aria-label={TIPS.kategori} onMouseEnter={() => setOpenTip('kategori')} style={{ color: '#6b7280', fontWeight: 600, fontSize: 12, marginLeft: 6, cursor: 'help', padding: 0, display: 'inline-block' }}>?</span>
                  {openTip === 'kategori' && (
                    <div role="dialog" aria-label="Informasi pengisian" style={{ position: 'absolute', top: 6, right: 8, background: '#fff', color: '#111', border: '1px solid #e5e7eb', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', padding: '8px 10px', maxWidth: 280, zIndex: 20 }}>
                      <div style={{ fontSize: 12 }}>{TIPS.kategori}</div>
                      <button onClick={() => setOpenTip(null)} aria-label="Tutup" style={{ position: 'absolute', top: 4, right: 6, background: 'transparent', border: 'none', color: '#6b7280', cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </th>
                <th style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
                  Tujuan
                  <span className="tooltip-trigger" role="button" aria-label={TIPS.tujuan} onMouseEnter={() => setOpenTip('tujuan')} style={{ color: '#6b7280', fontWeight: 600, fontSize: 12, marginLeft: 6, cursor: 'help', padding: 0, display: 'inline-block' }}>?</span>
                  {openTip === 'tujuan' && (
                    <div role="dialog" aria-label="Informasi pengisian" style={{ position: 'absolute', top: 6, right: 8, background: '#fff', color: '#111', border: '1px solid #e5e7eb', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', padding: '8px 10px', maxWidth: 280, zIndex: 20 }}>
                      <div style={{ fontSize: 12 }}>{TIPS.tujuan}</div>
                      <button onClick={() => setOpenTip(null)} aria-label="Tutup" style={{ position: 'absolute', top: 4, right: 6, background: 'transparent', border: 'none', color: '#6b7280', cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </th>
                <th style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
                  Perusahaan
                  <span className="tooltip-trigger" role="button" aria-label={TIPS.perusahaan} onMouseEnter={() => setOpenTip('perusahaan')} style={{ color: '#6b7280', fontWeight: 600, fontSize: 12, marginLeft: 6, cursor: 'help', padding: 0, display: 'inline-block' }}>?</span>
                  {openTip === 'perusahaan' && (
                    <div role="dialog" aria-label="Informasi pengisian" style={{ position: 'absolute', top: 6, right: 8, background: '#fff', color: '#111', border: '1px solid #e5e7eb', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', padding: '8px 10px', maxWidth: 280, zIndex: 20 }}>
                      <div style={{ fontSize: 12 }}>{TIPS.perusahaan}</div>
                      <button onClick={() => setOpenTip(null)} aria-label="Tutup" style={{ position: 'absolute', top: 4, right: 6, background: 'transparent', border: 'none', color: '#6b7280', cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </th>
                <th style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
                  No. Telepon/HP
                  <span className="tooltip-trigger" role="button" aria-label={TIPS.telepon} onMouseEnter={() => setOpenTip('telepon')} style={{ color: '#6b7280', fontWeight: 600, fontSize: 12, marginLeft: 6, cursor: 'help', padding: 0, display: 'inline-block' }}>?</span>
                  {openTip === 'telepon' && (
                    <div role="dialog" aria-label="Informasi pengisian" style={{ position: 'absolute', top: 6, right: 8, background: '#fff', color: '#111', border: '1px solid #e5e7eb', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', padding: '8px 10px', maxWidth: 280, zIndex: 20 }}>
                      <div style={{ fontSize: 12 }}>{TIPS.telepon}</div>
                      <button onClick={() => setOpenTip(null)} aria-label="Tutup" style={{ position: 'absolute', top: 4, right: 6, background: 'transparent', border: 'none', color: '#6b7280', cursor: 'pointer' }}>✕</button>
                    </div>
                  )}
                </th>
                ...
                <tr key={idx}>
                  ...
                  <td>
                    <input
                      type="text"
                      className="form-input"
                      value={row.kapalAngkut}
                      onChange={(e) => handleRowChange(idx, 'kapalAngkut', e.target.value)}
                      placeholder="Nama/jenis kapal angkut..."
                    />
                  </td>
                  <td>
                    <SmartSelect
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => handleRowChange(idx, 'perusahaan', e.target.value)}
                  ...
                  <td>
                    <input
                      ...
                      className="form-input"
                      value={row.telepon}
                      onChange={(e) => handleRowChange(idx, 'telepon', e.target.value)}
