...
const FasilitasFungsionalPemadamKebakaran = () => {
  ...
  const JENIS_FASILITAS_OPTIONS = ['Mobil Pemadam', 'Hydrant', 'Selang', 'Pompa', 'APAR', 'Lain-Lain']

  const TIPS = {
    pelabuhan: 'Pilih pelabuhan tempat fasilitas berada.',
    tahunPembuatan: 'Tahun pembuatan (tahun saja).',
    jenisFasilitas: 'Pilih jenis fasilitas pemadam kebakaran.',
    jumlah: 'Jumlah unit yang tersedia.',
    rencanaPengembanganJumlah: 'Rencana penambahan jumlah unit.',
    kapasitas: 'Kapasitas layanan (misal liter/menit).',
  ...
  const isRequiredEmpty = createIsRequiredEmpty(formData, {
    textKeys: ['pelabuhan','tahunPembuatan','jenisFasilitas','kondisi'],
    numericKeys: ['jumlah','kapasitas','rencanaPengembanganKapasitas']
  })
  const saveDisabled = createIsSaveDisabled(isRequiredEmpty, ['pelabuhan','tahunPembuatan','jenisFasilitas','jumlah','kapasitas','rencanaPengembanganKapasitas','kondisi'])
  ...
  return (
    <div className="section-card">
      <FormHeader
        title="Fasilitas Fungsional - Pemadam Kebakaran"
        description="Form untuk mengelola fasilitas pemadam kebakaran"
        rightSlot={(
      ...
      <form className="fasilitas-form" onSubmit={(e) => e.preventDefault()}>
        <div className="form-grid">
          ...
          <div className={`form-group ${isRequiredEmpty('kondisi') ? 'error' : ''} ${isRequiredEmpty('kondisi') && doShake ? 'shake' : ''}`} style={{ position: 'relative' }} onMouseLeave={() => setOpenTip(null)}>
            ...
            <input type="text" name="kondisi" value={formData.kondisi} onChange={handleInputChange} className="form-input" required />
          </div>
...
export default FasilitasFungsionalPemadamKebakaran